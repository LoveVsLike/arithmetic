...
/**
 * 邻接矩阵模型类 https://segmentfault.com/a/1190000002685939
 * 
 * @author dream
 *
 */
public class AdjacentMatrix<T> {
	...
	private void broadFirstSearch(boolean[] isVisited, int i) {
		...
		System.out.print(getValueByIndex(i) + " ");// 访问结点i
		isVisited[i] = true;
		queue.addLast(i);// 结点入队列
		while (!queue.isEmpty()) {
			u = ((Integer) queue.removeFirst()).intValue();
			w = getFirstNeighbor(u);
			while (w != -1) {
				if (!isVisited[w]) {// 访问该结点
					System.out.print(getValueByIndex(w) + "  ");
					isVisited[w] = true;// 标记已被访问
					queue.addLast(w);// 入队列
				}
				w = getNextNeighbor(u, w);// 寻找下一个邻接结点
			}
		}
	}
